...
`;
  ...
  const exportAsJSON = () => {
    const dataStr = JSON.stringify(mvpPlan, null, 2);
    const dataUri = 'data:application/json;charset=utf-8,'+ encodeURIComponent(dataStr);

    const exportFileDefaultName = `mvp-plan-${Date.now()}.json`;
  ...
  const exportAsImage = async () => {
    ...
    if (element) {
      try {
        ...
        link.download = `mvp-plan-${Date.now()}.png`;
...
`;
    ...
    const dataUri = 'data:text/markdown;charset=utf-8,' + encodeURIComponent(readmeContent);
    const linkElement = document.createElement('a');
    linkElement.setAttribute('href', dataUri);
    linkElement.setAttribute('download', `MVP-README-${Date.now()}.md`);
    linkElement.click();
  };
