...
// This line imports necessary modules and defines the AdminDashboard component, handling authentication, navigation, and rendering different admin sections.
...
function ApiKeysManagement() {
  ...
  const apiKeys = apiKeysData?.gemini || [];
        ...
        {apiKeys.length > 0 && (
          <div className="space-y-2">
            ...
            {apiKeys.map((key: any, index: number) => (
              <div key={key.id || index} className="flex justify-between items-center p-3 bg-slate-700 rounded-lg">
                <div className="flex flex-col">
                  <span className="text-slate-300 font-mono">
                    {key.key ? `${key.key.substring(0, 8)}...${key.key.substring(key.key.length - 8)}` : '***masked***'}
                  </span>
